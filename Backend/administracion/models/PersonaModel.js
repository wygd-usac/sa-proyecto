...
Persona.create = (newPersona, result) => {
    conexion.query("INSERT INTO Person SET ?", newPersona, (err,res) => {
        ...
        console.log("se creo un nuevo jugador o DT: ", {id: res.insertId, ...newPersona});
...
Persona.updateById = (id, persona, result) => {
    conexion.query(
      ...
      [persona.name,persona.lastname, persona.birthday, persona.status, persona.photo, persona.id_stand,persona.type, persona.id_team, persona.nationality ,id],
      (err, res) => {
        if (err) {
          console.log("error: ", err);
          result(null, err);
...
Persona.updateStatus = (id, status, result) => {
    conexion.query(
      "UPDATE Person SET status = ? WHERE id_person = ?",status,id,
      (err, res) => {
        if (err) {
          console.log("error: ", err);
          result(null, err);
        ...
        console.log("se actualizo el estado de jugador o DT: ", { id: id, ...persona });
        result(null, { id: id, ...persona });
      }
    );
};
